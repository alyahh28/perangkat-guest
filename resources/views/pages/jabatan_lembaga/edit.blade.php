                ...
                <div class="card unified-form-card">
                    <div class="card-header">
                        <h4 class="mb-0 text-white">
                            <i class="fa fa-edit me-2"></i>Edit Data Jabatan Lembaga
                        </h4>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('jabatan-lembaga.update', $jabatan->jabatan_id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                <label for="lembaga_id" class="form-label fw-semibold">Lembaga <span class="text-danger">*</span></label>
                                <select class="form-select form-control-lg @error('lembaga_id') is-invalid @enderror"
                                    ...
                                    @foreach($lembaga as $item)
                                        <option value="{{ $item->lembaga_id }}" {{ old('lembaga_id', $jabatan->lembaga_id) == $item->lembaga_id ? 'selected' : '' }}>
                                            {{ $item->nama_lembaga }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control form-control-lg @error('nama_jabatan') is-invalid @enderror"
                                       id="nama_jabatan" name="nama_jabatan"
                                       value="{{ old('nama_jabatan', $jabatan->nama_jabatan) }}" required maxlength="100"
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-4">
                                ...
                                <button type="submit" class="btn btn-add btn-lg">
                                    <i class="fa fa-save me-2"></i>Update Jabatan
                                </button>
                            </div>
                        </form>
                    </div>
