                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('lembaga.update', $lembaga->lembaga_id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-4 border-bottom pb-4">
                                ...
                                <div class="mb-3">
                                    <label for="singkatan" class="form-label fw-semibold">Singkatan (Opsional)</label>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="alamat" name="alamat" rows="2">{{ old('alamat', $lembaga->alamat) }}</textarea>
                                </div>
                                                    ...
                                                    <div class="position-relative">
                                                        ...
                                                        <div class="position-absolute top-0 end-0 p-2">
                                                            <div class="form-check bg-white rounded p-1 border">
                                                                <input type="checkbox" name="remove_media[]" value="{{ $media->media_id }}" class="form-check-input ms-0" id="rm_{{ $media->media_id }}">
                                                                <label for="rm_{{ $media->media_id }}" class="text-danger small fw-bold ps-1">Hapus</label>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="card-body p-2">
                                                        <small class="text-muted d-block">Caption:</small>
                                                        <input type="text" name="captions[{{ $media->media_id }}]" value="{{ $media->caption }}" class="form-control form-control-sm mb-1">

                                                        <small class="text-muted d-block">Urutan:</small>
                                                        <input type="number" name="sort_orders[{{ $media->media_id }}]" value="{{ $media->sort_order }}" class="form-control form-control-sm">
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
